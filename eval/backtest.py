...
def load_data():
    """
    Load data from local CSVs (datas/backtest) if available, falling back to yfinance (2015-Now).
    Enforces using 'Close' price for all calculations and executions.
    """
    print("⏳ Loading Data (2005-Now)...")
    start_date = '2005-01-01' 
    data_dir = "datas/backtest" 
    
    # --- Helper: Flatten yfinance MultiIndex ---
    def load_ticker_close(ticker, start):
        # 1. Try Local CSV first
        csv_path = os.path.join(data_dir, f"{ticker}.csv")
        # Handle special tickers like ^TNX -> TNX.csv
        if ticker == '^TNX':
            csv_path = os.path.join(data_dir, "TNX.csv")
        ...
        if os.path.exists(csv_path):
            try:
                ...
                if 'Close' in df.columns:
                    ...
                    return df['Adj Close']
                elif 'VALUE' in df.columns: # FRED data style sometimes
                     return df['VALUE']
                else:
                    return df.iloc[:, 0] # Fallback
            except Exception as e:
                print(f"   ⚠️ Local file error: {e}, falling back to download.")
        ...
        try:
            ...
            print("Done.")
            # Handle MultiIndex columns (e.g. ('Close', 'QQQ'))
            if isinstance(df.columns, pd.MultiIndex):
                # Try to extract 'Close' level
                try:
                    return df.xs('Close', axis=1, level=0).iloc[:, 0]
                except:
                    # Fallback: remove levels
    ...
    qqq_close = load_ticker_close('QQQ', start_date)
    shv_close = load_ticker_close('SHV', start_date)
    sgov_close = load_ticker_close('SGOV', '2020-01-01') # SGOV started later
    
    # 2. Download Macro Data (US10Y)
    print("⏳ Downloading ^TNX (US10Y) data...")
    tnx_close = load_ticker_close('^TNX', start_date)
    ...
    safe_asset_close = sgov_close.reindex(qqq_close.index).combine_first(
        shv_close.reindex(qqq_close.index)
    )

    # 4. Construct Main DataFrame
    data = pd.DataFrame(index=qqq_close.index)
    data['QQQ_Close'] = qqq_close
    data['SGOV_Close'] = safe_asset_close
    data['US10Y'] = tnx_close.reindex(data.index).ffill() # Align rates
